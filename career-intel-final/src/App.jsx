...
import { Target, FileText, Bookmark, Map, Zap } from "lucide-react";
...
const NAV = [
  ...
  { id:"deploy",       label:"Deploy guide", icon:Map },
];
...
export default function App() {
  ...
  const oppCount = (opps?.internships?.length||0) + (opps?.startups?.length||0) + (opps?.grad_programs?.length||0);
  ...
  return (
    <div className="shell">
        ...
        <div className="logo">
          <div className="logo-mark">
            ...
            <div>
              ...
              <div className="logo-sub">Personal · v2</div>
            </div>
          </div>
        </div>
        ...
        <div className="side-bot">
          <div className="user-row">
            ...
            <div style={{flex:1,minWidth:0}}>
              ...
              {profile?.gpa && (
                <div style={{fontSize:".68rem",color:"var(--amber)",fontFamily:"var(--mono)"}}>GPA {profile.gpa}</div>
              )}
            </div>
            {profile && (
              <button className="btn ghost xs" onClick={reset} title="Clear all data">Reset</button>
            )}
          </div>
        </div>
      </aside>
      ...
      <main className="main">
        ...
        {page==="opportunities" && <OpportunitiesPage profile={profile} opps={opps} onRefresh={setOpps} saved={saved} onSave={toggleSave}/>}
